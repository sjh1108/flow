...
**내용 검사는 "위장"만 잡습니다.** 정직하게 `.exe`라고 이름 붙은 진짜 실행 파일은 확장자 정책이 판단합니다. 그러지 않으면 `exe` 체크를 해제해도 실행 파일이 계속 막혀서, 체크박스가 지키지 못할 약속을 하게 됩니다. 확장자가 아예 없는 실행 파일은 정책이 손댈 대상이 없으므로 그대로 거부합니다.
