...
**컨테이너마다 DB 계정이 하나씩이고 서로의 것을 모릅니다.** 이전에는 앱이 `SPRING_FLYWAY_USER`로 마이그레이터 자격증명을 들고 있었는데, 그 계정은 `ALL PRIVILEGES`라 앱을 장악한 쪽이 환경변수를 읽어 그대로 쓸 수 있었습니다. 계정을 나눠도 한쪽이 다른 쪽 비밀번호를 갖고 있으면 분리가 아닙니다.
