...
저장소를 Vercel에 연결하면 PR마다 프리뷰가 생기는데, **프리뷰 URL은 운영과 다른 오리진**입니다. `CORS_ALLOWED_ORIGINS`에는 운영 도메인만 있으므로 프리뷰에서 화면을 열면 API 호출이 전부 차단됩니다. 증상은 위와 같습니다 — 화면은 뜨고 오류 배너가 뜨며 체크박스가 0개.

**고장이 아니라 설정대로 도는 것이고, 의도적으로 열어두지 않았습니다.** 여는 방법 둘 다 대가가 있기 때문입니다.
...
- **CORS에 프리뷰 오리진 추가** — `allowedOriginPatterns`로 가능하지만 `*.vercel.app`을 열면 **남의 Vercel 앱도 이 API를 부를 수 있습니다**

프리뷰로 UI를 확인할 필요가 실제로 생기면 그때 정하세요. **다만 "프리뷰 주소 하나만 허용 목록에 넣으면 된다"는 방법은 없습니다.** Vercel의 안정적인 프리뷰 별칭은 브랜치에서 파생되므로(`flow-git-<브랜치>-<스코프>.vercel.app`, 이름이 길면 잘리고 해시가 붙습니다) 브랜치마다 주소가 달라집니다. 상시로 쓰려면 프리뷰 전용 브랜치를 정해 그 주소를 고정하거나, 패턴을 열되 범위를 좁게 잡아야 합니다.
