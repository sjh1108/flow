...
그 결과 **로컬 브라우저 검증이 운영 API를 때립니다.** 추측이 아니라 관찰입니다 — 브라우저가
`https://api.algoj.duckdns.org`로 나가는 것을 보고 난 뒤에 고쳤습니다. `ui-verify.mjs`는
문서가 파싱되는 동안 이 태그를 자기가 겨냥한 주소로 고쳐 씁니다. `main.js`가 모듈이라 파싱
이후에 실행되므로, 페이지 스크립트보다 먼저 설치된 `MutationObserver`가 제때 잡습니다.

요청 가로채기(`page.route`)로 HTML을 바꿔 치는 쪽이 더 곧아 보이지만 **쓰지 않았습니다.**
미리 설치된 Chromium이 playwright 패키지와 리비전이 달라서, 가로채기를 켜면 페이지의 첫 API
호출이 응답도 실패도 없이 매달립니다(`page.evaluate`로 같은 주소를 부르면 12ms). 이 판단에
이르기까지의 오진은 `tasks/lessons.md` 12번에 적었습니다.
