...
> **경합 한계 (수용).** 사용량을 **요청당 한 번** 읽으므로, 같이 출발한 동시 요청들은 같은 합계를 보고 각자 그 여유를 씁니다. 따라서 초과량을 묶는 것은 파일 하나가 아니라 **요청 하나가 담을 수 있는 전부**입니다.
>
> **초과량 ≤ 동시 요청 수 × (요청당 최대 파일 수 × 최대 파일 크기)** = 기본값에서 동시 요청 수 × **200MB**
>
> 10GB 쿼터에 10개 요청이 동시에 가득 찬 배치를 올리면 약 2GB까지 넘길 수 있습니다. 락 없이 이 창을 줄이는 방법은 `max-files-per-request`나 `max-file-size`를 낮추는 것이고, 사용량을 요청당이 아니라 **파일당** 다시 읽으면 배치 배수(10배)만큼 좁아지는 대신 파일마다 쿼리가 한 번씩 더 붙습니다. 정확히 막으려면 예약 슬롯을 트랜잭션으로 잡아야 하는데 그러면 업로드가 직렬화됩니다.
>
> 디스크 자체는 `min-free-space`가 따로 지키므로 이 초과가 쓰기 실패로 이어지지는 않습니다. 그래도 **한 파일 크기로 묶인다고 적었던 이전 서술은 10배 낙관적이었고**(3차 리뷰 지적), 위 수치가 실제 한계입니다.
