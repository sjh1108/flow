...
> **경합 한계 (수용).** 동시 요청은 각자 시작 시점의 사용량으로 판단하므로 상한을 잠시 넘길 수 있습니다. 초과분은 (동시 요청 수 × 최대 파일 크기)로 묶이고 다음 요청이 곧바로 막습니다. 예약 슬롯을 트랜잭션으로 잡으면 모든 업로드가 직렬화되는데, 이 규모에서 대가가 이득보다 큽니다. 200개 한도 경합(4-1)과 같은 판단입니다.
