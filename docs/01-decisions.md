...
**왜 이대로 두는가.** 초과의 대가가 **정확도이지 정합성이 아니기** 때문입니다. `min-free-space`가 디스크에 여유를 남기고, 그럼에도 공간이 모자라 쓰기가 실패하면 그 실패는 **아무것도 남기지 않고 끝납니다**(`.part` + 실패 시 양쪽 정리).

> **다만 이것도 보장은 아닙니다.** 앞서 "이 초과가 쓰기 실패로 이어지지는 않는다"고 적었는데 **너무 강한 서술이었습니다**(리뷰 지적). `min-free-space`도 **요청당 한 번** 읽고, 그 값은 다른 프로세스 때문에 읽는 순간 이미 과거이며, 겹친 요청들은 같은 여유를 중복해서 씁니다. 조회가 실패하면 검사 자체가 생략됩니다. 즉 ENOSPC 확률을 **낮출 뿐 배제하지 못합니다.** 실제 보장은 "쓰기가 실패해도 쓰레기가 남지 않는다"까지입니다.
...
507 프론트 건이 특히 뼈아픈데, PR 본문에 "본문 형태가 동일하므로 클라이언트는 상태 코드로 분기하지 않아도 된다"고 적어놓고 **정작 클라이언트는 분기하고 있었고 제가 그걸 고치지 않았습니다.** 서버 계약을 바꾸면서 그것을 읽는 쪽을 확인하지 않은 것입니다.
