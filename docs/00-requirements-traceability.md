...
| 4-3 | 저장소 고갈 방어 | 쿼터 10GB + 디스크 최소 여유 1GB, 보존 30일 정리, 고아·부분 파일 정리 (`StorageBudget`, `StorageMaintenanceService`, `LocalFileStorage`) | `StorageQuotaIntegrationTest`, `StorageMaintenanceIntegrationTest`, `LocalFileStorageTest` |
...
| 계층 | 건수 | 실행 방법 |
|---|---|---|
| 백엔드 단위·통합 테스트 | **179** | `cd backend && ./gradlew test` |
