...
|---|---|
| **200** | 1건 이상 성공 |
| **422** | 전부 거부 — 파일 자체가 정책에 걸림. 같은 파일을 다시 보내도 결과는 같음 |
| **507** | 전부 거부 — 저장 공간 부족. 파일에는 문제가 없으며 정리 후 다시 시도하면 성공 |

세 경우 **본문 형태가 동일**하므로 클라이언트는 상태 코드로 분기하지 않고 `results`를 그대로 렌더하면 됩니다. 422와 507을 나눈 이유는 재시도 여부가 다르기 때문입니다 — 용량 때문에 거부된 것을 422로 알리면 멀쩡한 파일을 포기하게 됩니다.
