...
**문자는 영문·숫자·`_`·`-`만 씁니다.** SQL 리터럴에 들어가므로 따옴표나 역슬래시가 있으면
비밀번호가 약해지는 게 아니라 **문이 깨져서 계정이 아예 만들어지지 않습니다.**
`01-users.sh`가 형식을 검사해 그 전에 명확한 메시지로 실패합니다.

```bash
openssl rand -hex 32
```

셸·Compose·SQL 세 곳의 escape를 각각 맞추는 것보다 형식을 좁히는 쪽이 단순하고, 128비트면
충분합니다.
