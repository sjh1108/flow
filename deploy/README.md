...
이제 **상시 실행되는 앱은 `extguard_app` 자격증명만** 받습니다. root도 마이그레이터도
갖지 않고, `migrate`는 마이그레이터만 받습니다. CI가 검증하는 것도 정확히 이 경계입니다.

> "컨테이너마다 계정이 하나씩"이라고는 말할 수 없습니다. `mysql`은 계정을 만들어야 하므로
> 세 비밀번호를 다 받고 `grants`는 root를 받습니다. 둘 다 부팅에만 관여하고 하나는 곧
> 종료하지만, 그래도 **오래 떠 있으면서 요청을 받는 것은 앱뿐**이라는 게 이 분리의 요점입니다.
...
- 앱은 루프백에만 바인딩합니다. **호스트 포트는 `.env`의 `APP_HOST_PORT`로 정하고
  기본값이 8080**입니다. 이 기계에서 이미 8080을 쓰는 것이 있으면 바꾸세요 — compose는
  다른 포트로 물러서지 않고 `port is already allocated`로 **기동에 실패**합니다.
  컨테이너 안은 항상 8080이라 Caddy 설정과 헬스체크는 영향을 받지 않습니다
- Caddy는 `with-tls` 프로파일 뒤에 있어 **기본적으로 뜨지 않습니다.** 기존 리버스 프록시가
  이미 80/443을 쓰고 있다면 그대로 두고, 거기에 `127.0.0.1:${APP_HOST_PORT}`로 가는 항목만
  추가하세요
...
**그중 둘은 형식이 강제됩니다** — `MIGRATOR_PASSWORD`와 `APP_DB_PASSWORD`는 영문·숫자·`_`·`-`만
허용하고, `01-users.sh`가 검사해 아니면 즉시 실패합니다. 이 둘은 SQL 리터럴에 보간되므로
따옴표나 역슬래시가 있으면 비밀번호가 약해지는 게 아니라 **문이 깨져서 계정이 아예
만들어지지 않습니다.** 셸·Compose·SQL 세 곳의 escape를 각각 맞추는 것보다 형식을 좁히는
쪽이 단순하고, 128비트면 충분합니다.

`MYSQL_ROOT_PASSWORD`는 **검사하지 않습니다.** SQL 리터럴에 들어가지 않고 클라이언트가
`MYSQL_PWD`로 읽으므로 같은 경로가 아닙니다. 권장이 없는 건 아니고, 강제가 없을 뿐입니다.
...
없어야 하는 문을 둘 다** 실행합니다. CI도 같은 스크립트를 돌립니다.

**할 수 없어야 하는 쪽이 진짜 단언입니다.** 성공 쪽만 검사하면 권한을 전부 열어놓아도
통과하는데, 그게 바로 이 경계가 막으려는 상황입니다.
